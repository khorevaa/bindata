...
Процедура ПриСозданииОбъекта(Знач ВходящееИмяКласса, Знач ПутьККаталогуКлассов)
	...
	ИмяКлассаФайла = "bindata_"+ВходящееИмяКласса;
...
Процедура ДобавитьФайл(Знач ПутьКФайлу, Знач КорневойКаталог = "") Экспорт
	...
	НаборФайлов.Добавить(НовыйФайлНабора(ИмяФайла, ПолучитьXMLДанные(ПутьКФайлу)))

КонецПроцедуры

Функция ПолучитьXMLДанные(ПутьКФайлу)

	ДвоичныеДанныеФайла = Новый ДвоичныеДанные(ПутьКФайлу);
	XMLДанные = XMLСтрока(ДвоичныеДанныеФайла);
	XMLДанные = XMLСтрока(ПутьКФайлу);
	Возврат XMLДанные;
